...
1. **L1 — Memory hardening**: retention and cleanup guidelines for `.braindrain/` and continual-learning index; document in `.devdocs` + README when finalized.
...
3. **Admin / ops tools**: draft flows for operator-facing tools (see plan docs); keep `config/hub_config.yaml` and launcher paths in sync with README and `.devdocs/TODOS.md`.
4. **L2 / L3 memory tiers**: explicit semantics and optional `~/.braindrain/memory/` — design only until L1 is stable.

## Out of scope for this file

- Secrets, hostnames, tokens, or per-user MCP paths. Use `.env.example` and README for patterns, not live values.
