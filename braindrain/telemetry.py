...
# Handles spaces by matching more chars but stopping at common delimiters.
_PATH_RE = re.compile(
...
_GENERIC_SECRET_RE = re.compile(
    r"(?i)\b(password|secret|apikey|api_key|token)\b\s*[:=]\s*([\"']?[a-zA-Z0-9_\-\.]{6,}[\"']?)"
)
        ...
        def _do_sanitize(val: Any) -> Any:
            if isinstance(val, str):
                ...
                if (
                    ...
                    and "AKIA" not in val
                    and "xox" not in val
                    and not any(k in val.lower() for k in ("password", "secret", "apikey", "api_key", "token"))
                ):
                    return val
