...
# Handles spaces by matching until common delimiters (quotes, comma, newline, tab, semicolon, colon)
_PATH_RE = re.compile(r"(/Users/|/Volumes/|/home/|/root/)[^'\",\n\t;:]+")
# API Keys: OpenAI/Anthropic (sk-), Groq (gsk_), HuggingFace (hf_), Google AI (AIza), AWS (AKIA/ASIA), Slack (xox)
_KEY_RE = re.compile(
...
class TelemetrySession:
    ...
    def _sanitize_data(self, data: Any) -> Any:
        ...
        def _do_sanitize(val: Any, key: str | None = None) -> Any:
            # Redact sensitive dictionary keys unless the value is numeric (observability)
            if (
                key
                and any(
                    s in key.lower()
            ...
            if isinstance(val, dict):
                return {k: _do_sanitize(v, k) for k, v in val.items()}
