...
# Paths: /Users/..., /Volumes/..., /home/..., /root/... (supports spaces; stops at common delimiters)
_PATH_RE = re.compile(r"(/Users/[^'\",\n\t;]+|/Volumes/[^'\",\n\t;]+|/home/[^'\",\n\t;]+|/root/[^'\",\n\t;]+)")
# API Keys: OpenAI/Anthropic (sk-), Groq (gsk_), HuggingFace (hf_), Google AI (AIza), AWS (AKIA), Slack (xoxb/xoxp)
_KEY_RE = re.compile(
    r"(sk-[a-zA-Z0-9-]{20,}|gsk_[a-zA-Z0-9]{20,}|hf_[a-zA-Z0-9]{20,}|AIza[a-zA-Z0-9_-]{35,}|AKIA[0-9A-Z]{16}|xox[baprs]-[0-9a-zA-Z-]{10,})"
)
# Generic secrets: password: ..., apikey=... (supports optional quotes)
_GENERIC_SECRET_RE = re.compile(
    r"(?i)\b(password|secret|apikey|api_key|token)\b\s*[:=]\s*(\"[^\"]+\"|'[^']+'|[^\s,;\"']+)",
)
        ...
        def _do_sanitize(val: Any) -> Any:
            if isinstance(val, str):
                ...
                if (
                    ...
                    # Check if generic secret words are present before running the regex
                    if not any(
                        word in v_lower
                        for word in ("password", "secret", "apikey", "api_key", "token")
                    ):
                        return val

                val = _PATH_RE.sub("[REDACTED_PATH]", val)
                val = _KEY_RE.sub("[REDACTED_KEY]", val)
                val = _GENERIC_SECRET_RE.sub(r"\1: [REDACTED_SECRET]", val)
                return val
