...
# API Keys: OpenAI/Anthropic (sk-), Groq (gsk_), HuggingFace (hf_), Google AI (AIza), AWS (AKIA/ASIA), Slack (xoxb/xoxp/...)
_KEY_RE = re.compile(
    r"(sk-[a-zA-Z0-9-]{20,}|gsk_[a-zA-Z0-9]{20,}|hf_[a-zA-Z0-9]{20,}|AIza[a-zA-Z0-9_-]{35,}|A[KS]IA[A-Z0-9]{16,}|xox[bparc]-[a-zA-Z0-9-]{12,})",
    re.IGNORECASE,
)

# Generic secrets: password, secret, token, api_key, etc. followed by : or =
_GENERIC_SECRET_RE = re.compile(
    r"((?:password|secret|token|apikey|api_key|pass)(?:[\s_-]*))([:=])([\s]*)(['\"]?)([^\s'\",;]+)(\4)",
    re.IGNORECASE,
)
    ...
    def _sanitize_data(self, data: Any) -> Any:
        ...
        def _do_sanitize(val: Any) -> Any:
            if isinstance(val, str):
                ...
                # a potential sensitive path, API key, or generic secret keyword.
                v_lower = val.lower()
                if (
                    "/" not in val
                ...
                val = _GENERIC_SECRET_RE.sub(r"\1\2\3\4[REDACTED_SECRET]\4", val)
                return val
