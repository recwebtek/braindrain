...
# Generic secrets: password=value, "secret": "value"
        ...
        def _do_sanitize(val: Any) -> Any:
            ...
            if isinstance(val, dict):
                ...
                for k, v in val.items():
                    # Redact values for sensitive keys, but only if they are strings.
                    # This preserves numeric telemetry (like tokens_saved) while hiding actual secrets.
                    if isinstance(k, str):
                        k_lower = k.lower()
                        if any(
                            s in k_lower
