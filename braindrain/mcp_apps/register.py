...
_TOKEN_APP = AppConfig(resource_uri=TOKEN_DASHBOARD_URI, prefers_border=True)
_PLAN_APP = AppConfig(resource_uri=PLAN_BOARD_URI, prefers_border=True)
...
def register_mcp_app_tools(
    ...
    *,
    ...
    async def show_token_dashboard(path: str = ".") -> ToolResult:
        ...
        """
        payload = build_token_dashboard_payload(telemetry, path=path)
        return ToolResult(
            content=[{"type": "text", "text": "Token dashboard ready."}],
            structured_content={"dashboard": payload},
        )
    ...
    async def poll_token_dashboard(path: str = ".") -> ToolResult:
        ...
        """
        payload = build_token_dashboard_payload(telemetry, path=path)
        return ToolResult(structured_content={"dashboard": payload})
    ...
    async def show_plan_board(path: str = ".") -> ToolResult:
        ...
        return ToolResult(
            content=[{"type": "text", "text": "Plan board ready."}],
            structured_content={"dashboard": payload},
        )
