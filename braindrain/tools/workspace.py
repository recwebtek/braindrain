...
async def prime_workspace_impl(
    prime_workspace_fn,
...
def init_project_memory_impl(
    initialize_project_memory_fn, telemetry, path: str = ".", dry_run: bool = False
) -> dict:
    try:
        ...
        telemetry.log_error(
            f"init_project_memory exception: {e}", context={"path": path, "dry_run": dry_run}
        )
        return {"ok": False, "error": str(e)}
