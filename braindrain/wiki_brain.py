...
class WikiBrain:
    ...
    def detect_contradiction(
        ...
        *,
        ...
        """
        Detect contradictions by comparing the new record with the last 25 records
        of the same class. Optimized to avoid full object hydration and use short-circuiting logic.
        """
        with self._connect() as conn:
            rows = conn.execute(
                """
        ...
        for row in rows:
            ...
            # Short-circuit: only check content similarity if titles overlap significantly
            # This avoids expensive similarity checks for unrelated records.
