...
class WikiBrain:
    ...
    def decay_records(self, *, now: float | None = None) -> dict[str, Any]:
        """
        Apply importance decay to all active records using a single bulk UPDATE.
        Leverages SQLite's POWER function for ~2.6x performance improvement.
        """
        ...
        with self._connect() as conn:
            if self.decay_half_life_days <= 0:
                ...
                # decay = importance * POWER(0.5, (current - anchor) / (86400.0 * half_life_days))
                cursor = conn.execute(
                    """
                    UPDATE brain_records
                    SET
