...
class WikiBrain:
    ...
    def decay_records(self, *, now: float | None = None) -> dict[str, Any]:
        ...
        Optimized with a bulk SQL UPDATE and POWER() function (~2.3x speedup).
        """
        ...
        with self._connect() as conn:
            # Bulk update using exponential decay formula:
            # importance = importance * 0.5 ^ (delta_days / half_life_days)
            # COALESCE(NULLIF(updated_at, 0), created_at) handles fallback to created_at
            cursor = conn.execute(
                """
                UPDATE brain_records
                SET
